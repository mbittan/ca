...
/**	\file	Instruction.h
	...
	\author	Hajjem - Heydemann - Girault
*/
...
private:
   ...
   Instruction *_next; //instruction suivante dans le code
   Instruction *_prev; //précédente instruction dans le code
 ...
 */
   ...
   bool reads_in(int dst);
   bool writes_in(int dst);
...
/**	\brief	get the information if there is dependance RAW between the current instruction and i2	
	\return return true if there is a RAW dependance between the current instruction and i2 

*/	bool is_dep_RAW1(Instruction *i2);
/**	\brief	get the information if there is dependance RAW between the current instruction and the first source operand of i2	
	\return return true if there is a RAW dependance between the current instruction and the first source register operand of i2 
...
/**	\brief	add a type of a dep with a predecessor instruction to the dependance type list
 */
...
/**	\brief	add a type of a dep with a successor instruction to list of the dependance type of successors
 */
   void add_succ_dep(dep*);

   list<dep*>::iterator succ_begin();
   list<dep*>::iterator succ_end();
/**	\brief	get the dependance type with ith successor  instruction of the current instruction
 */
